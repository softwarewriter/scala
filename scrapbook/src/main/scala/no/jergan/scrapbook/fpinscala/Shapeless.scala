...
import shapeless.{::, Generic, HList, HNil, Lens, Poly1, lens}
...
object Shapeless {
  ...
  def main(args: Array[String]): Unit = {
    ...
    object print extends Poly1 {
      ...
      implicit def caseInt: Case[Int] {type Result} = at[Int] { i => println(s"int: " + i)}

      implicit def caseString = at[String] { s => println(s"string: " + s)}

      implicit def casePelle = at[Person] {p => println(s"pelle: " + p.name)}
    }
